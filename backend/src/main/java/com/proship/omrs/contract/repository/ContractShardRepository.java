...
public interface ContractShardRepository extends JpaRepository<ContractMainShard,Long> {
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            "on c.contract_id = csct.contract_id" +
                    " INNER JOIN participant_act act on c.act_id = act.id where c.gig_id in ( :gigIds)  " +
                    "and csct.nexttransactiontime > current_date and " +
                    "csct.cancel = false and act.active = true " +
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            ...
            "and act_id not in (1,2,2576,2577) and act.active = true and c.nexttransactiontime > current_date ",
            nativeQuery = true)
    Set<BigInteger> findParticipantByGigAndRankGroup(@Param("gigIds") Set<Long> gigIds, @Param("positions") Set<Long> positions);
