...
public interface ContractShardRepository extends JpaRepository<ContractMainShard,Long> {
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            ...
            "JOIN participant_name_tts pnt on act.participant_id = pnt.participant_id where c.gig_id in ( :gigIds)  " +
             "and csct.nexttransactiontime > now() and csct.cancel = false and act.active = true " +
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            "on c.contract_id = csct.contract_id INNER JOIN participant_act act on c.act_id = act.id " +
            "JOIN participant_name_tts pnt on act.participant_id = pnt.participant_id where c.gig_id in ( :gigIds)  " +
            "and c.position = :position and csct.nexttransactiontime > now() and " +
            "csct.cancel = false and act.active = true and act_id not in (1,2,2576,2577) and " +
            "act.active = true and c.nexttransactiontime > now() and pnt.nexttransactiontime>now()",
            nativeQuery = true)
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            ...
            "now() and  csct.cancel = false and act.active = true and act_id not in (1,2,2576,2577) " +
            "and act.active = true and c.nexttransactiontime > now() and pnt.nexttransactiontime>now()",
            nativeQuery = true)
    ...
    @Query(value = "select act.id from contract_main_shard c join contract_status_cancel_tts csct " +
            ...
            "and c.position in :positions and csct.nexttransactiontime > now() and " +
            "csct.cancel = false and act.active = true AND con.non_ps = FALSE " +
            "and act_id not in (1,2,2576,2577) and act.active = true and c.nexttransactiontime > now() and " +
            "pnt.nexttransactiontime>now()",
            nativeQuery = true)
